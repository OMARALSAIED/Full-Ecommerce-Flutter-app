...
class DataPusher {
  ...
  Future<void> pushProducts() async {
    try {
      for (var product in DummyData.dummyProducts) {
        await _firestore.collection('products').doc(product.id).set(product.toJson());
        print('Product ${product.title} added successfully.');
      }
    } catch (e) {
  ...
  Future<void> pushBanners() async {
    try {
      for (var banner in DummyData.banners) {
        await _firestore.collection('banners').add(banner.toJson());
